...
| 10 | 连续脚本标签、关闭与横向滚动；连续圆角磨砂主体包含标签、编辑器及结果；Safari 式原生玻璃活动标签，非活动标题 secondary；居中标题与悬停/焦点关闭叠层 | Safari 最终深浅 GUI、中英文长名、窄工作区溢出选中自动滚动、⇧⌘W 关闭与非活动关闭通过；用户确认悬停正常浮现/消失、标题不动 |
...
最终按用户最新参考采用连续圆角工作主体，统一包含脚本标签栏、编辑器与查询结果。外层一层系统 regularMaterial，16 pt 圆角、8 pt 内侧安全间距，避免圆角裁切行号/滚动条/结果。活动标签使用公开 glassEffect(.regular) 的 Capsule 胶囊形状，不再制作底边贴合的文件夹型标签。参考图片是访达的背景与层次参考，未声称复用 Safari 或访达内部控件。

编辑器与结果区使用 12 pt 圆角内面板，左右标题栏统一为 40 pt。顶部标签与 SQL 工具栏覆盖在原生滚动编辑器上，使用系统 .bar 材质采样下方内容；NSScrollView.contentInsets 为首行、选区显露与滚动条保留顶部空间，滚动文字可进入磨砂层后方。新增原生测试验证首行、向上跳转到中间行不会被工具栏遮挡，重排和手动阅读位置仍保留。

非活动文字为系统 secondary、活动文字为 primary，标题固定宽度居中。关闭控件以 28 pt 叠层在悬停或键盘焦点时浮现，仅按钮下方采用局部原生材质，不参与标签尺寸计算。隐藏时按钮不在视图树中；保留上下文菜单、辅助功能关闭动作与 ⇧⌘W 菜单快捷键。活动标签改变时自动滚入视野。
