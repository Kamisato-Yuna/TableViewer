...
最终按用户最新参考采用连续圆角工作主体，统一包含脚本标签栏、编辑器与查询结果。外层一层系统 regularMaterial，16 pt 圆角、8 pt 内侧安全间距，避免圆角裁切行号/滚动条/结果。活动标签使用公开 glassEffect(.regular) 的完整圆角形状，不再制作底边贴合的文件夹型标签。参考图片是访达的背景与层次参考，未声称复用 Safari 或访达内部控件。
...
- Release 编译通过（本地 Xcode beta / macOS SDK，ad hoc 签名）；最新构建记录为 tv053-rounded-workspace.log。
