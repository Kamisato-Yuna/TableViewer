...
GitHub Pages 使用 Actions 发布，工作流为 `.github/workflows/pages.yml`。网站相关文件推送至 main，正式 Release 发布、编辑、删除，或手动运行工作流时，读取最新正式 Release 并构建。版本号、日期、ZIP 大小、下载链接和前三条发布要点直接来自 GitHub；发布要点按纯文本转义，完整说明链接至 Release。未找到正式 arm64 ZIP 时停止构建，保留已发布站点。
